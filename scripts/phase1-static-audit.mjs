...
if (!trainingEmailProcessor.includes('NCR Suite V2.29.23')) {
  errors.push('Le processeur documentaire Formation doit annoncer NCR Suite V2.29.23.');
}
...
const visualIdentitiesMigration = read('supabase/migrations/098_identity_logos_profile_avatar.sql');
const avatarCropMigration = read('supabase/migrations/099_profile_avatar_crop_release.sql');
const premiumWorkspaceMigration = read('supabase/migrations/100_premium_workspace_polish.sql');
...
const solutionLayoutFixMigration = read('supabase/migrations/107_solution_layout_fix_release.sql');
const enterpriseNotificationShortcutMigration = read('supabase/migrations/108_enterprise_notification_shortcut.sql');
...
const publicFlowTransmissionMigration = read('supabase/migrations/120_public_flow_transmission.sql');
const securityAgentLogbookUxReleaseMigration = read('supabase/migrations/123_security_logbook_photos_quick_texts.sql');
const securityLogbookPdfSource = read('src/features/security/logbookPdf.ts');
const securityLogbookPageSource = read('src/pages/SecurityLogbookPage.tsx');
const securityLogbookPhotoDisplayFixMigration = read('supabase/migrations/124_security_logbook_photo_display_fix.sql');
const securityLogbookPdfPhotosMigration = read('supabase/migrations/125_security_logbook_pdf_photos.sql');
const securityDashboardSource = read('src/pages/SecurityDashboardPage.tsx');
const securityVacationHardeningMigration = read('supabase/migrations/126_security_vacation_hardening.sql');
const securityPremiumPresenceMigration = read('supabase/migrations/127_security_premium_shift_presence.sql');
...
if (!indexHtml.includes('/ncr-suite-showcase-v2923.css')
    || !indexHtml.includes('/ncr-suite-app-v2923.css')
    || !indexHtml.includes('ncr-style-guard')
    || !indexHtml.includes('ncr:css-recovery-v2.29.23')
    || !showcaseGenerator.includes('ncr-suite-showcase-v2923.css')
    || !showcaseGenerator.includes('ncr-suite-app-v2923.css')
    || !viteConfig.includes('codeSplitting: false')
    || !viteConfig.includes("entryFileNames: 'ncr-suite-app-v2923.js'")
    || !publicStyles.includes('--ncr-styles-ready: 1')) {
  errors.push('La protection V2.29.23 contre les fragments /assets indisponibles est incomplete.');
}
if (!cloudflareHeaders.includes('Content-Type: text/css; charset=utf-8')
    || !cloudflareHeaders.includes('/ncr-suite-showcase-v2923.css')
    || !cloudflareHeaders.includes('/ncr-suite-app-v2923.css')) {
  errors.push('Les en-tetes CSS Cloudflare V2.29.23 sont incomplets.');
}
if (!runtimeConfig.includes("APP_VERSION = '2.29.23'")
    || !runtimeConfig.includes("ncr-suite-shell-v2.29.23-training-closure-documents-premium")
    || !serviceWorker.includes("ncr-suite-shell-v2.29.23-training-closure-documents-premium")
    || !serviceWorker.includes("'/ncr-suite-showcase-v2923.css'")
    || !serviceWorker.includes("'/ncr-suite-app-v2923.css'")
    || !serviceWorker.includes("'/ncr-suite-app-v2923.js'")
    || !serviceWorker.includes("'/brand/ncr-suite-application-icon-v281.png'")) {
  errors.push('La version ou le cache PWA V2.29.23 est incoherent.');
}
if (read('src/main.tsx').includes("import './styles.css'")) {
  errors.push('Le style complet V2.29.23 ne doit pas etre fragmente dans /assets.');
}
...
if (!premiumCockpitMigration.includes("'2.26.2'")
    ...
    || !publicStyles.includes('.training-quality-period.training-quality-period-mobile { display:none !important; }')
    || !publicStyles.includes('.training-quality-period.training-quality-period-mobile { display:grid !important;grid-column:1/-1; }')
...
if (!visualIdentitiesMigration.includes("'2.26.3'")
    || !visualIdentitiesMigration.includes('ncr-suite-shell-v2.26.3-visual-identities')
    || !visualIdentitiesMigration.includes('add column if not exists avatar_url')
    || !visualIdentitiesMigration.includes("'profile-avatars'")
    || !visualIdentitiesMigration.includes('profile_avatars_insert_own')
    || !visualIdentitiesMigration.includes('platform_release_state')
    || !appShell.includes('organization.logo_url')
    || !appShell.includes('org.logo_url')
    || !appShell.includes("from('profile-avatars')")
    || !appShell.includes("from('user_profiles')")
    || !appShell.includes('profile-avatar-upload')
    || !appShell.includes('<AvatarContent')
    || !publicStyles.includes('.context-switcher-icon.has-image')
    || !publicStyles.includes('.profile-avatar-upload')
    || !publicStyles.includes('.profile-avatar-toast')) {
  errors.push('Les identites visuelles V2.26.3 ou leur stockage securise sont incomplets.');
}
if (!avatarCropMigration.includes("'2.26.4'")
    || !avatarCropMigration.includes('ncr-suite-shell-v2.26.4-avatar-crop')
    || !avatarCropMigration.includes('avatar_url')
    || !avatarCropMigration.includes("'profile-avatars'")
    || !avatarCropMigration.includes('platform_release_state')
    || !appShell.includes('AVATAR_CROP_SIZE')
    || !appShell.includes('handleAvatarCropPointerMove')
    || !appShell.includes("canvas.toBlob(resolve, 'image/webp'")
    || !appShell.includes("canvas.toBlob(resolve, 'image/jpeg'")
    || !appShell.includes('Utiliser la photo')
    || !publicStyles.includes('.avatar-crop-overlay')
    || !publicStyles.includes('.avatar-crop-viewport')
    || !publicStyles.includes('.avatar-crop-zoom')
    || !publicStyles.includes('.user-avatar > img')) {
  errors.push('Le recadrage de photo de profil V2.26.4 est incomplet.');
}
if (!premiumWorkspaceMigration.includes("'2.26.5'")
    || !premiumWorkspaceMigration.includes('ncr-suite-shell-v2.26.5-premium-workspace')
    || !premiumWorkspaceMigration.includes('platform_release_state')
    || !appShell.includes('app-shell app-shell-v265')
    || !publicStyles.includes('.app-shell-v265 .page > .page-header')
    || !publicStyles.includes('.app-shell-v265 .main-nav a.active::before')
    || !publicStyles.includes('.app-shell-v265 .primary-button')
    || !publicStyles.includes('.app-shell-v265 .stat-card:hover')
    || !publicStyles.includes('.app-shell-v265 .empty-state')
    || !publicStyles.includes('.app-shell-v265 .client-table th')
    || !publicStyles.includes('@keyframes premium-page-enter')
    || !publicStyles.includes('@media (prefers-reduced-motion: reduce)')) {
  errors.push('La finition premium transversale V2.26.5 est incomplete.');
}
if (!compactNavigationMigration.includes("'2.26.6'")
    || !compactNavigationMigration.includes('ncr-suite-shell-v2.26.6-compact-navigation')
    || !compactNavigationMigration.includes('security_addon_catalog')
    || !compactNavigationMigration.includes('training_module_catalog')
    || !appShell.includes('navigation-search')
    || !appShell.includes('grouped-navigation')
    || !appShell.includes('Modules disponibles')
    || !appShell.includes('app-shell-v266')
    || !subscriptionPage.includes("data.business_type === 'securite' && ['decouverte', 'essentielle'].includes(data.subscription.plan)")
    || !publicStyles.includes('.navigation-group.available')
    || !publicStyles.includes('.navigation-search')) {
  errors.push('La navigation compacte et la coherence des modules V2.26.6 sont incompletes.');
}
if (!commercialReadinessMigration.includes("'2.27.0'")
    || !commercialReadinessMigration.includes('ncr-suite-shell-v2.27.0-commercial-readiness')
    || !commercialReadinessMigration.includes('platform_commercial_validation_runs')
    || !commercialReadinessMigration.includes('platform_commercial_readiness_report')
    || !commercialReadinessMigration.includes('store_platform_commercial_validation')
    || !commercialReadinessMigration.includes('platform_commercial_validation_history')
    || !commercialReadinessMigration.includes("data_retention_mode='preserve'")
    || !commercialReadinessMigration.includes('set search_path = public,pg_catalog')
    || !commercialReadinessPanel.includes("supabase.rpc('platform_commercial_readiness_report'")
    || !commercialReadinessPanel.includes("supabase.rpc('store_platform_commercial_validation'")
    || !commercialReadinessPanel.includes('Clôturer la recette')
    || !platformAdminPage.includes("activeSection === 'validation'")
    || !platformAdminPage.includes('<AdminCommercialReadinessPanel />')
    || !appShell.includes('app-shell-v270')
    || !publicStyles.includes('.admin-commercial-readiness-stages')
    || !publicStyles.includes('.admin-commercial-readiness-decision')) {
  errors.push('La recette commerciale V2.27.0 est incomplete.');
}
if (!interactionsMigration.includes("'2.27.1'")
    || !interactionsMigration.includes('ncr-suite-shell-v2.27.1-interactions')
    || !interactionsMigration.includes('platform_release_state')
    || !appShell.includes('app-shell-v271')
    || !publicStyles.includes('.premium-route-stage')
...
if (!premiumSolutionPagesMigration.includes("'2.28.1'")
    || !premiumSolutionPagesMigration.includes('ncr-suite-shell-v2.28.1-premium-solution-pages')
    || !premiumSolutionPagesMigration.includes('platform_release_state')
    || !publicSolutionPage.includes('public-solution-v281')
    || !publicSolutionPage.includes('data-solution-reveal')
    || !publicSolutionPage.includes('IntersectionObserver')
    || !publicSolutionPage.includes('/brand/ncr-suite-application-icon-v281.png')
    || !publicStyles.includes('NCR Suite V2.28.1 - pages metier premium et interactions')
    || !publicStyles.includes('@keyframes public-solution-card-enter')
...
if (!solutionLayoutFixMigration.includes("'2.28.3'")
    || !solutionLayoutFixMigration.includes('ncr-suite-shell-v2.28.3-solution-layout-fix')
    || !solutionLayoutFixMigration.includes('platform_release_state')
    || !publicSolutionPage.includes('public-solution-v283')
    || !publicStyles.includes('NCR Suite V2.28.3 - lisibilite des cartes et rythme vertical desktop')
    || !publicStyles.includes('@media (min-width: 1001px)')
    || !publicStyles.includes('article.public-solution-feature-card')
    || !publicStyles.includes('"feature-header"')
    || !publicStyles.includes('"outcome-label outcome-title"')
    || !publicStyles.includes('article.feature-card-1')) {
  errors.push('Le correctif desktop des cartes et du rythme vertical V2.28.3 est incomplet.');
}
if (!enterpriseNotificationShortcutMigration.includes("'2.28.4'")
    || !enterpriseNotificationShortcutMigration.includes('ncr-suite-shell-v2.28.4-enterprise-notification-shortcut')
    || !enterpriseNotificationShortcutMigration.includes('platform_release_state')
    || !appShell.includes('app-shell-v284')
    || !appShell.includes('enterprise-notification-shortcut desktop')
    || !appShell.includes('enterprise-notification-shortcut mobile')
    || !appShell.includes('to="/notifications"')
    || !publicStyles.includes('NCR Suite V2.28.4 - raccourci permanent des notifications entreprise')
    || !publicStyles.includes('.app-shell-v284 .enterprise-notification-shortcut')) {
  errors.push('Le raccourci permanent des notifications entreprise V2.28.4 est incomplet.');
}
if (!universalNotificationAccessMigration.includes("'2.28.5'")
    || !universalNotificationAccessMigration.includes('ncr-suite-shell-v2.28.5-universal-notification-access')
    || !universalNotificationAccessMigration.includes('platform_release_state')
    || !moduleAccess.includes("const UNIVERSAL_MODULE_PATHS = new Set(['/notifications'])")
    || !moduleAccess.includes('const isUniversalModule = UNIVERSAL_MODULE_PATHS.has(normalized)')
    || !moduleAccess.includes('!isUniversalModule && !isCoiffureLoyaltyBase')
    || !moduleAccess.includes('!isUniversalModule && organization.plan')) {
  errors.push('La protection universelle de la rubrique Notifications V2.28.5 est incomplete.');
}
if (!publicSolutionsMenuMigration.includes("'2.28.6'")
    || !publicSolutionsMenuMigration.includes('ncr-suite-shell-v2.28.6-solutions-menu')
    || !publicSolutionsMenuMigration.includes('platform_release_state')
    || !publicHeader.includes('publicSeoPages.json')
    || !publicHeader.includes('public-solutions-trigger')
    || !publicHeader.includes('public-solutions-strip')
    || !publicHeader.includes('aria-expanded={solutionsOpen}')
    || !publicStyles.includes('NCR Suite V2.28.6 - menu horizontal des solutions metier')
    || !publicStyles.includes('grid-template-columns: repeat(5, minmax(0, 1fr))')
    || !publicStyles.includes('.public-solutions-strip > a:hover')) {
  errors.push('Le menu horizontal des pages metier V2.28.6 est incomplet.');
}
const favicon48Dimensions = readPngDimensions('public/icons/favicon-48.png');
const favicon96Dimensions = readPngDimensions('public/icons/favicon-96.png');
if (!googleSearchFaviconMigration.includes("'2.28.7'")
    || !googleSearchFaviconMigration.includes('ncr-suite-shell-v2.28.7-google-favicon')
    || !googleSearchFaviconMigration.includes('platform_release_state')
    || !fs.existsSync(path.join(root, 'public/favicon.ico'))
    || favicon48Dimensions?.width !== 48
    || favicon48Dimensions?.height !== 48
    || favicon96Dimensions?.width !== 96
    || favicon96Dimensions?.height !== 96
    || !indexHtml.includes('href="/favicon.ico"')
    || !indexHtml.includes('sizes="96x96" href="/icons/favicon-96.png"')
    || !indexHtml.includes('sizes="48x48" href="/icons/favicon-48.png"')
    || !serviceWorker.includes("'/favicon.ico'")
    || !serviceWorker.includes("'/icons/favicon-48.png'")
    || !serviceWorker.includes("'/icons/favicon-96.png'")
    || !cloudflareHeaders.includes('/favicon.ico')
...
if (!publicUiSpacingFixMigration.includes("'2.29.2'")
    ...
    || !publicStyles.includes('.public-home-v292 .public-flow-grid article:nth-child(2)')
    || !publicStyles.includes('.public-home-v292 .public-site-footer .public-footer-brand img')) {
  errors.push('Les corrections de cadrage et de lisibilite V2.29.2 sont incompletes.');
}
...
if (!securityLogbookPdfSource.includes('PHOTOS JOINTES')
    || !securityLogbookPdfSource.includes('embedLogbookPhoto')
    || !securityLogbookPageSource.includes('loadSecurityLogbookPhotoMap(organization.id, missionEntries.map')) {
  errors.push('Les photos ne sont pas raccordees au PDF de main courante V2.29.10.');
}
if (!securityAgentLogbookUxReleaseMigration.includes("'2.29.8'")
    || !securityAgentLogbookUxReleaseMigration.includes('ncr-suite-shell-v2.29.8-security-logbook-photos')) {
  errors.push('La release V2.29.8 photos main courante / cache PWA est incomplete.');
}
if (!securityLogbookPhotoDisplayFixMigration.includes("'2.29.9'")
    || !securityLogbookPhotoDisplayFixMigration.includes('ncr-suite-shell-v2.29.9-security-logbook-photo-display')
    || !securityLogbookPhotoDisplayFixMigration.includes('can_read_security_logbook_photo_object')) {
  errors.push('La release V2.29.9 affichage photos main courante / cache PWA est incomplete.');
}
if (!securityLogbookPdfPhotosMigration.includes("'2.29.10'")
    || !securityLogbookPdfPhotosMigration.includes('ncr-suite-shell-v2.29.10-security-logbook-photo-display')) {
  errors.push('La release V2.29.10 photos dans le PDF de main courante / cache PWA est incomplete.');
}
if (!securityVacationHardeningMigration.includes("'2.29.11'")
    || !securityVacationHardeningMigration.includes('ncr-suite-shell-v2.29.11-security-vacation-hardening')
    || !securityVacationHardeningMigration.includes('enforce_single_active_security_shift')
    || !securityVacationHardeningMigration.includes('security_patrols_agent_insert')
    || !securityDashboardSource.includes('Terminer la vacation oubliée')
    || !securityDashboardSource.includes(".eq('status', 'in_progress').not('clocked_in_at', 'is', null).is('clocked_out_at', null)")) {
  errors.push('Le blindage des vacations Sécurité V2.29.11 est incomplet.');
}
if (!securityPremiumPresenceMigration.includes("'2.29.12'")
    || !securityPremiumPresenceMigration.includes('ncr-suite-shell-v2.29.12-security-premium-presence')
    || !securityPremiumPresenceMigration.includes('security_shift_proofs')
    || !securityPremiumPresenceMigration.includes('set_security_shift_presence_event_premium')
    || !securityPremiumPresenceMigration.includes('get_security_shift_handover')
    || !securityDashboardSource.includes('Fin de mission proche')
    || !securityDashboardSource.includes('SecurityShiftPresenceSheet')) {
  errors.push('La prise / fin de poste Premium V2.29.12 est incomplete.');
}
if (!publicFlowTransmissionMigration.includes("'2.29.6'")
    ...
    || !publicStyles.includes('@keyframes public-flow-card-receive-v296')
    || !publicStyles.includes('@keyframes public-solution-interface-enter-v296')
    || !publicStyles.includes('@keyframes public-solution-interface-float-v296')
    || !publicStyles.includes('.public-home-v296 .public-flow-transmission::after')) {
  errors.push('La transmission progressive V2.29.6 du flux public est incomplete.');
}
...
for (const publicAnimation of [
  ...
  'public-flow-transmission-pulse-v296',
  'public-flow-card-receive-v296'
]) {
...
if (!trainingPremiumPdfMultiline.includes("text.split('\\n')") || !trainingPremiumPdfMultiline.includes('page.drawText(line')) {
  errors.push('Le correctif PDF multi-ligne V2.29.23 du socle premium est absent.');
}
if (!trainingDocumentProcessor.includes("pdfFontSafeText(value, font).split('\\n')") || !trainingDocumentProcessor.includes('page.drawText(lineText')) {
  errors.push('Le correctif PDF multi-ligne V2.29.23 des documents automatiques est absent.');
}
if (!trainingAttendancePdfMultiline.includes("replace(/\\n+/g, ' · ')") ) {
  errors.push('La sécurisation des champs mono-ligne des émargements V2.29.23 est absente.');
}
