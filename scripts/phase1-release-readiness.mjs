...
if (pkg.version !== '2.28.0') failures.push('package.json doit annoncer la V2.28.0.');
if (!runtime.includes(`APP_VERSION = '${pkg.version}'`)) failures.push('La version runtime ne correspond pas au paquet.');
if (!runtime.includes(`PWA_CACHE_NAME = '${expectedCache}'`)) failures.push('Le cache runtime V2.28.0 est incohérent.');
if (!serviceWorker.includes(`const CACHE = '${expectedCache}'`)) failures.push('Le Service Worker V2.28.0 est incohérent.');
...
if (!fs.existsSync(path.join(root, 'public/ncr-suite-showcase-v280.css'))) {
  failures.push('La feuille de style critique V2.28.0 n’a pas été générée.');
}
if (!fs.existsSync(path.join(root, 'public/ncr-suite-app-v280.css'))) {
  failures.push('La feuille de style complète V2.28.0 n’a pas été générée.');
}
