...
if (pkg.version !== '2.29.24') failures.push('package.json doit annoncer la V2.29.24.');
if (!runtime.includes(`APP_VERSION = '${pkg.version}'`)) failures.push('La version runtime ne correspond pas au paquet.');
if (!runtime.includes(`PWA_CACHE_NAME = '${expectedCache}'`)) failures.push('Le cache runtime V2.29.24 est incohérent.');
if (!serviceWorker.includes(`const CACHE = '${expectedCache}'`)) failures.push('Le Service Worker V2.29.24 est incohérent.');
...
if (!fs.existsSync(path.join(root, 'public/ncr-suite-showcase-v2924.css'))) {
  failures.push('La feuille de style critique V2.29.21 n’a pas été générée.');
}
if (!fs.existsSync(path.join(root, 'public/ncr-suite-app-v2924.css'))) {
