...
const expectedCache = `ncr-suite-shell-v${pkg.version}-security-presence-photo-release-fix`;
...
if (pkg.version !== '2.29.14') failures.push('package.json doit annoncer la V2.29.14.');
if (!runtime.includes(`APP_VERSION = '${pkg.version}'`)) failures.push('La version runtime ne correspond pas au paquet.');
if (!runtime.includes(`PWA_CACHE_NAME = '${expectedCache}'`)) failures.push('Le cache runtime V2.29.14 est incohérent.');
if (!serviceWorker.includes(`const CACHE = '${expectedCache}'`)) failures.push('Le Service Worker V2.29.14 est incohérent.');
...
requireText('scripts/generate-public-showcase-css.mjs', [
  'ncr-suite-showcase-v2914.css',
  'ncr-suite-app-v2914.css',
...
if (!fs.existsSync(path.join(root, 'public/ncr-suite-showcase-v2914.css'))) {
  failures.push('La feuille de style critique V2.29.14 n’a pas été générée.');
}
if (!fs.existsSync(path.join(root, 'public/ncr-suite-app-v2914.css'))) {
  failures.push('La feuille de style complète V2.29.14 n’a pas été générée.');
}
...
requireText('vite.config.ts', [
  'codeSplitting: false',
  "entryFileNames: 'ncr-suite-app-v2914.js'"
]);
