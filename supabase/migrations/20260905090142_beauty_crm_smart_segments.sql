...
CREATE OR REPLACE FUNCTION public.beauty_crm_segment_clients(p_organization_id uuid, p_company_id uuid, p_segment text DEFAULT 'all'::text, p_search text DEFAULT NULL::text, p_limit integer DEFAULT 100, p_offset integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'public', 'private', 'pg_catalog'
AS $function$
  select private.beauty_crm_segment_clients(
    p_organization_id,p_company_id,p_segment,p_search,p_limit,p_offset
  );
$function$


CREATE OR REPLACE FUNCTION public.beauty_crm_segments_dashboard(p_organization_id uuid, p_company_id uuid)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'public', 'private', 'pg_catalog'
AS $function$
  select private.beauty_crm_segments_dashboard(p_organization_id,p_company_id);
$function$



revoke all on function private.beauty_crm_segments_dashboard(uuid,uuid) from public,anon;
grant execute on function private.beauty_crm_segments_dashboard(uuid,uuid) to authenticated,service_role;
revoke all on function public.beauty_crm_segments_dashboard(uuid,uuid) from public,anon;
grant execute on function public.beauty_crm_segments_dashboard(uuid,uuid) to authenticated,service_role;

revoke all on function private.beauty_crm_segment_clients(uuid,uuid,text,text,integer,integer) from public,anon;
grant execute on function private.beauty_crm_segment_clients(uuid,uuid,text,text,integer,integer) to authenticated,service_role;
revoke all on function public.beauty_crm_segment_clients(uuid,uuid,text,text,integer,integer) from public,anon;
grant execute on function public.beauty_crm_segment_clients(uuid,uuid,text,text,integer,integer) to authenticated,service_role;
