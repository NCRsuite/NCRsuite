...
do $$
begin
  if not exists (select 1 from pg_constraint where conrelid='public.beauty_company_loyalty_settings'::regclass and contype='p') then
    alter table public.beauty_company_loyalty_settings
      add constraint beauty_company_loyalty_settings_pkey primary key (organization_id,company_id);
  end if;
  if not exists (select 1 from pg_constraint where conrelid='public.beauty_company_loyalty_settings'::regclass and conname='beauty_company_loyalty_settings_organization_fk') then
    alter table public.beauty_company_loyalty_settings
      add constraint beauty_company_loyalty_settings_organization_fk
      foreign key (organization_id) references public.organizations(id) on delete cascade;
  end if;
  if not exists (select 1 from pg_constraint where conrelid='public.beauty_company_loyalty_settings'::regclass and conname='beauty_company_loyalty_settings_company_fk') then
    alter table public.beauty_company_loyalty_settings
  ...
  if not exists (select 1 from pg_constraint where conrelid='public.beauty_company_loyalty_settings'::regclass and conname='beauty_company_loyalty_settings_created_by_fk') then
    alter table public.beauty_company_loyalty_settings
...
drop policy if exists beauty_company_loyalty_settings_rpc_only on public.beauty_company_loyalty_settings;
create policy beauty_company_loyalty_settings_rpc_only
on public.beauty_company_loyalty_settings
...
set company_id=(select c.company_id from public.clients c where c.organization_id=r.organization_id and c.id=r.client_id)
where r.company_id is null
  and exists(select 1 from public.clients c where c.organization_id=r.organization_id and c.id=r.client_id and c.company_id is not null);
...
CREATE OR REPLACE FUNCTION private.coiffure_client_portal_company_feature_enabled(p_organization_id uuid, p_company_id uuid)
 RETURNS boolean
 LANGUAGE sql
...
AS $function$
  select exists(
    ...
    where o.id=p_organization_id
      ...
      and (
        case when p_company_id is null then
          coalesce((select s.portal_enabled from public.coiffure_loyalty_settings s where s.organization_id=o.id),true)
        else
          exists(
            select 1
          ...
          and coalesce((
            select s.portal_enabled
            from public.beauty_company_loyalty_settings s
            where s.organization_id=o.id and s.company_id=p_company_id
          ),true)
        end
      )
  );
...
CREATE OR REPLACE FUNCTION private.enforce_beauty_loyalty_scope()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path = public, pg_catalog
AS $function$
...
begin
  if tg_table_name='beauty_company_loyalty_settings' then
    if not exists(
      select 1
      from public.organization_companies c
  ...
  if new.company_id is distinct from v_client_company then
    raise exception 'Le client et la fidélité n’appartiennent pas à la même enseigne.';
  end if;
  ...
  if tg_table_name='coiffure_loyalty_ledger' then
    if new.appointment_id is not null
       and not exists(
         ...
         where a.id=new.appointment_id
           ...
           and a.company_id is not distinct from new.company_id
       ) then
      raise exception 'Le rendez-vous et le ledger n’appartiennent pas au même périmètre.';
    end if;

    if new.reward_id is not null
       and not exists(
         select 1
         from public.coiffure_loyalty_rewards r
      ...
      raise exception 'La récompense et le ledger n’appartiennent pas au même périmètre.';
    end if;
    ...
    if new.appointment_id is not null
      ...
      raise exception 'Le rendez-vous et l’état fidélité n’appartiennent pas au même périmètre.';
    end if;
  end if;
...
begin
  ...
  if not private.coiffure_client_portal_company_feature_enabled(
      v_inv.organization_id,
      (select c.company_id from public.clients c where c.organization_id=v_inv.organization_id and c.id=v_inv.client_id)
    ) then raise exception 'L’espace client est actuellement désactivé.'; end if;
...
CREATE OR REPLACE FUNCTION public.create_coiffure_client_portal_invitation(p_organization_id uuid, p_client_id uuid, p_email text, p_display_name text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path = public, private, pg_catalog
...
begin
  ...
  if not private.coiffure_client_portal_company_feature_enabled(
      p_organization_id,
      (select c.company_id from public.clients c where c.organization_id=p_organization_id and c.id=p_client_id)
    ) then raise exception 'L’espace client de cette enseigne est désactivé.'; end if;
...
CREATE OR REPLACE FUNCTION public.current_coiffure_client_portal_accounts()
 RETURNS TABLE(account_id uuid, organization_id uuid, client_id uuid, organization_name text, organization_logo_url text, organization_primary_color text, client_name text, display_name text, unread_rewards integer, last_seen_at timestamp with time zone)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path = public, private, pg_catalog
AS $function$
  select
    ...
    (select count(*)::integer
      ...
      where r.organization_id=a.organization_id
        ...
        and r.company_id is not distinct from c.company_id
        and r.status='available'),
    a.last_seen_at
  from public.coiffure_client_portal_accounts a
  ...
  where a.user_id=auth.uid()
    and a.status='active'
    and c.status='active'
    and private.coiffure_client_portal_company_feature_enabled(a.organization_id,c.company_id)
...
AS $function$
  ...
  where i.token_hash=extensions.digest(trim(p_token),'sha256')
    and private.coiffure_client_portal_company_feature_enabled(i.organization_id,c.company_id)
  limit 1;
$function$;
...
begin
  ...
  where a.user_id=auth.uid()
    ...
    and private.coiffure_client_portal_company_feature_enabled(a.organization_id,oc.id)
...
begin
  ...
  return jsonb_build_object(
    'available', private.coiffure_client_portal_company_feature_enabled(v_appointment.organization_id,v_appointment.company_id) and v_email is not null,
...
begin
  ...
  if not private.coiffure_client_portal_company_feature_enabled(v_appointment.organization_id,v_appointment.company_id) then
...
CREATE OR REPLACE FUNCTION public.update_coiffure_client_portal_profile(p_account_id uuid, p_birth_date date, p_birthday_consent boolean, p_marketing_opt_in boolean)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path = public, private, pg_catalog
...
declare
  v_account public.coiffure_client_portal_accounts%rowtype;
  v_company_id uuid;
  v_allow_birthdate_edit boolean:=true;
begin
  if not public.is_coiffure_client_portal_account(p_account_id) then
    raise exception 'Accès refusé.';
  end if;
  ...
  if v_company_id is not null then
    perform private.ensure_beauty_company_loyalty_settings(v_account.organization_id,v_company_id,null);
    select s.allow_client_birthdate_edit into v_allow_birthdate_edit
    from public.beauty_company_loyalty_settings s
    where s.organization_id=v_account.organization_id
  ...
  update public.clients
  set birth_date=case when coalesce(v_allow_birthdate_edit,true) then p_birth_date else birth_date end,
      birthday_consent=coalesce(p_birthday_consent,false),
      marketing_opt_in=coalesce(p_marketing_opt_in,false),
      updated_at=now()
  where organization_id=v_account.organization_id
...
revoke all on function private.ensure_beauty_company_loyalty_settings(uuid,uuid,uuid) from public;
grant execute on function private.ensure_beauty_company_loyalty_settings(uuid,uuid,uuid) to authenticated,service_role;
revoke all on function private.expire_beauty_company_loyalty_rewards(uuid,uuid,uuid) from public;
grant execute on function private.expire_beauty_company_loyalty_rewards(uuid,uuid,uuid) to authenticated,service_role;
revoke all on function private.issue_beauty_company_threshold_rewards(uuid,uuid,uuid) from public;
grant execute on function private.issue_beauty_company_threshold_rewards(uuid,uuid,uuid) to authenticated,service_role;
revoke all on function private.ensure_beauty_company_birthday_reward(uuid,uuid,uuid) from public;
grant execute on function private.ensure_beauty_company_birthday_reward(uuid,uuid,uuid) to authenticated,service_role;
revoke all on function private.apply_beauty_company_welcome_benefit(uuid,uuid,uuid) from public;
grant execute on function private.apply_beauty_company_welcome_benefit(uuid,uuid,uuid) to authenticated,service_role;
...
revoke all on function private.process_beauty_company_appointment_loyalty(uuid,uuid,uuid,uuid,text,integer,uuid) from public;
...
revoke all on function private.enforce_beauty_loyalty_scope() from public;
revoke all on function private.coiffure_client_portal_company_feature_enabled(uuid,uuid) from public;
grant execute on function private.coiffure_client_portal_company_feature_enabled(uuid,uuid) to anon,authenticated,service_role;

revoke all on function public.expire_coiffure_loyalty_rewards(uuid,uuid) from public;
revoke execute on function public.expire_coiffure_loyalty_rewards(uuid,uuid) from anon,authenticated;
grant execute on function public.expire_coiffure_loyalty_rewards(uuid,uuid) to service_role;
...
for each row execute function private.enforce_beauty_loyalty_scope();

drop trigger if exists coiffure_loyalty_ledger_company_scope_guard on public.coiffure_loyalty_ledger;
create trigger coiffure_loyalty_ledger_company_scope_guard
before insert or update on public.coiffure_loyalty_ledger
for each row execute function private.enforce_beauty_loyalty_scope();

drop trigger if exists coiffure_loyalty_rewards_company_scope_guard on public.coiffure_loyalty_rewards;
create trigger coiffure_loyalty_rewards_company_scope_guard
before insert or update on public.coiffure_loyalty_rewards
for each row execute function private.enforce_beauty_loyalty_scope();

drop trigger if exists coiffure_appointment_loyalty_state_company_scope_guard on public.coiffure_appointment_loyalty_state;
create trigger coiffure_appointment_loyalty_state_company_scope_guard
