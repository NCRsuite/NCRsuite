create index if not exists appointments_company_client_no_show_idx
  on public.appointments(organization_id,company_id,client_id)
  where status='no_show';

CREATE OR REPLACE FUNCTION private.beauty_crm_workspace(p_organization_id uuid, p_company_id uuid, p_segment text DEFAULT 'all'::text, p_search text DEFAULT NULL::text, p_limit integer DEFAULT 100, p_offset integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
  ...
  with metrics as (
    select
      ...
      coalesce((
        ...
        where bc.organization_id=p_organization_id
          ...
          and bc.client_id=c.id
          and bc.consent_type='marketing'
      ...
      coalesce((
        ...
        where bc.organization_id=p_organization_id
          and bc.company_id=p_company_id
          and bc.client_id=c.id
          and bc.consent_type='birthday'
        order by bc.recorded_at desc,bc.id desc
        limit 1
      ),c.birthday_consent,false) as birthday_allowed,
      coalesce(completed.visit_count,0)::int as visit_count,
      coalesce(completed.total_spent_cents,0)::bigint as total_spent_cents,
      completed.first_visit,
      completed.last_visit,
      upcoming.next_appointment,
      coalesce(no_show.no_show_count,0)::int as no_show_count,
      case when coalesce(completed.visit_count,0)>=2 then
        round(
          (extract(epoch from (completed.last_visit-completed.first_visit))/86400.0)
          / greatest(1,completed.visit_count-1),
          1
        )
    ...
    left join lateral (
      select
        ...
        coalesce(sum(ap.amount_cents),0)::bigint as total_spent_cents,
        min(ap.starts_at) as first_visit,
        max(ap.starts_at) as last_visit
    ...
    left join lateral (
      select min(ap.starts_at) as next_appointment
      from public.appointments ap
      where ap.organization_id=p_organization_id
        and ap.company_id=p_company_id
        and ap.client_id=c.id
...
CREATE OR REPLACE FUNCTION public.beauty_crm_workspace(p_organization_id uuid, p_company_id uuid, p_segment text DEFAULT 'all'::text, p_search text DEFAULT NULL::text, p_limit integer DEFAULT 100, p_offset integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path = public,private,pg_catalog
AS $function$
  select private.beauty_crm_workspace(
    p_organization_id,p_company_id,p_segment,p_search,p_limit,p_offset
  );
$function$


revoke all on function private.beauty_crm_workspace(uuid,uuid,text,text,integer,integer) from public,anon;
grant execute on function private.beauty_crm_workspace(uuid,uuid,text,text,integer,integer) to authenticated,service_role;
revoke all on function public.beauty_crm_workspace(uuid,uuid,text,text,integer,integer) from public,anon;
grant execute on function public.beauty_crm_workspace(uuid,uuid,text,text,integer,integer) to authenticated,service_role;
