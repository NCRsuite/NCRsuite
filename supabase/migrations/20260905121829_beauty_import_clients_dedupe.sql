...
alter table public.organization_import_jobs
  drop constraint if exists organization_import_jobs_source_provider_check;
alter table public.organization_import_jobs
  add constraint organization_import_jobs_source_provider_check
  check (source_provider is null or source_provider in ('planity','booksy','treatwell','csv'));

create index if not exists organization_import_jobs_company_created_idx
  on public.organization_import_jobs(organization_id,company_id,created_at desc);

create table if not exists public.beauty_import_external_refs (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
  company_id uuid not null references public.organization_companies(id) on delete cascade,
  import_job_id uuid references public.organization_import_jobs(id) on delete set null,
  source_provider text not null check (source_provider in ('planity','booksy','treatwell','csv')),
  entity_type text not null check (entity_type in ('client','appointment')),
  external_id text not null,
  local_id uuid not null,
  created_at timestamptz not null default now(),
  unique(organization_id,company_id,source_provider,entity_type,external_id)
);

create index if not exists beauty_import_external_refs_local_idx
  on public.beauty_import_external_refs(organization_id,company_id,entity_type,local_id);

alter table public.beauty_import_external_refs enable row level security;
revoke all on public.beauty_import_external_refs from anon,authenticated;

create index if not exists clients_company_email_import_idx
  on public.clients(organization_id,company_id,lower(email))
  where email is not null;
create index if not exists services_company_name_import_idx
  on public.services(organization_id,company_id,lower(name));
create index if not exists staff_company_name_import_idx
  on public.staff(organization_id,company_id,lower(display_name));
create index if not exists sites_company_name_import_idx
  on public.organization_sites(organization_id,company_id,lower(name));
...
AS $function$
declare
  v_job_id uuid; v_row jsonb; v_inserted integer:=0; v_skipped integer:=0;
  ...
  v_existing_id uuid; v_client_id uuid; v_status text;
begin
  ...
  if jsonb_array_length(p_rows)<1 or jsonb_array_length(p_rows)>5000 then raise exception 'L import doit contenir entre 1 et 5 000 lignes.'; end if;
  if not exists(
  ...
  insert into public.organization_import_jobs(
    ...
    'processing',jsonb_array_length(p_rows),auth.uid(),jsonb_build_object('safe_dedupe',true)
  ...
  loop
    ...
    begin
      ...
      v_birth_date:=null; v_email_id:=null; v_phone_id:=null; v_name_id:=null; v_existing_id:=null;
      ...
      if v_email is not null then
        ...
        where organization_id=p_organization_id and company_id=p_company_id and lower(coalesce(email,''))=v_email
      ...
      select id into v_name_id from public.clients
      where organization_id=p_organization_id and company_id=p_company_id
        and lower(trim(first_name))=lower(v_first_name)
        and lower(trim(coalesce(last_name,'')))=lower(trim(coalesce(v_last_name,'')))
      order by created_at limit 1;
      if v_name_id is not null and not v_force then v_skipped:=v_skipped+1; continue; end if;
...
CREATE OR REPLACE FUNCTION private.beauty_import_normalize_phone(p_phone text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
...
begin
  ...
  if v like '0033%' and length(v)>=13 then
    return '0'||substring(v from 5);
  elsif v like '33%' and length(v)=11 then
...
CREATE OR REPLACE FUNCTION private.beauty_list_import_jobs(p_organization_id uuid, p_company_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'id',j.id,'import_type',j.import_type,'import_scope',j.import_scope,'source_provider',j.source_provider,
    'file_name',j.file_name,'status',j.status,'total_rows',j.total_rows,'inserted_rows',j.inserted_rows,
    'skipped_rows',j.skipped_rows,'error_rows',j.error_rows,'errors',j.errors,'metadata',j.metadata,
    'created_at',j.created_at,'completed_at',j.completed_at
  ) order by j.created_at desc),'[]'::jsonb)
  into v_result
  from (select * from public.organization_import_jobs
    where organization_id=p_organization_id and company_id=p_company_id
    order by created_at desc limit 30) j;
  return v_result;
end;
$function$;
  ...
  matches as (
    select i.*,
      (select c.id from public.clients c where c.organization_id=p_organization_id and c.company_id=p_company_id
         and i.email is not null and lower(coalesce(c.email,''))=i.email order by c.created_at limit 1) as email_client_id,
      (select c.id from public.clients c where c.organization_id=p_organization_id and c.company_id=p_company_id
         and i.phone_norm is not null and private.beauty_import_normalize_phone(c.phone)=i.phone_norm order by c.created_at limit 1) as phone_client_id,
      (select c.id from public.clients c where c.organization_id=p_organization_id and c.company_id=p_company_id
         and lower(trim(c.first_name))=lower(i.first_name)
         and lower(trim(coalesce(c.last_name,'')))=lower(trim(coalesce(i.last_name,''))) order by c.created_at limit 1) as name_client_id,
  ...
  classified as (
    select m.*,
      case
        ...
        when m.name_client_id is not null then 'possible_duplicate'
        else 'ready'
      end as status,
      case
        ...
        when m.name_client_id is not null then 'Même prénom et nom détectés : vérification recommandée.'
        else 'Prête à être importée.'
      end as reason,
      coalesce(case when m.email_client_id is not null and m.phone_client_id is not null and m.email_client_id<>m.phone_client_id
        then null else coalesce(m.email_client_id,m.phone_client_id,m.name_client_id) end,null) as matched_client_id
    from matches m
  ),
  with_client as (
    select c.*,existing.first_name as existing_first_name,existing.last_name as existing_last_name,
      existing.email as existing_email,existing.phone as existing_phone
    from classified c
  ...
  select jsonb_build_object(
    ...
    'items',coalesce(jsonb_agg(jsonb_build_object(
      ...
      'matched_client',case when matched_client_id is null then null else jsonb_build_object(
        ...
        'email',existing_email,'phone',existing_phone
      ) end
    ) order by line),'[]'::jsonb)
...
create index if not exists clients_company_phone_import_idx
  on public.clients(organization_id,company_id,private.beauty_import_normalize_phone(phone))
  where phone is not null;

CREATE OR REPLACE FUNCTION public.beauty_import_clients(p_organization_id uuid, p_company_id uuid, p_source_provider text, p_file_name text, p_rows jsonb)
 RETURNS jsonb
 LANGUAGE sql
 SET search_path = public,private,pg_catalog
AS $function$
  select private.beauty_import_clients(p_organization_id,p_company_id,p_source_provider,p_file_name,p_rows);
$function$;

CREATE OR REPLACE FUNCTION public.beauty_list_import_jobs(p_organization_id uuid, p_company_id uuid)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path = public,private,pg_catalog
AS $function$
  select private.beauty_list_import_jobs(p_organization_id,p_company_id);
$function$;

CREATE OR REPLACE FUNCTION public.beauty_preview_client_import(p_organization_id uuid, p_company_id uuid, p_source_provider text, p_rows jsonb)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path = public,private,pg_catalog
AS $function$
  select private.beauty_preview_client_import(p_organization_id,p_company_id,p_source_provider,p_rows);
$function$;

revoke all on function private.beauty_preview_client_import(uuid,uuid,text,jsonb) from public,anon;
grant execute on function private.beauty_preview_client_import(uuid,uuid,text,jsonb) to authenticated,service_role;
revoke all on function public.beauty_preview_client_import(uuid,uuid,text,jsonb) from public,anon;
grant execute on function public.beauty_preview_client_import(uuid,uuid,text,jsonb) to authenticated,service_role;

revoke all on function private.beauty_import_clients(uuid,uuid,text,text,jsonb) from public,anon;
grant execute on function private.beauty_import_clients(uuid,uuid,text,text,jsonb) to authenticated,service_role;
revoke all on function public.beauty_import_clients(uuid,uuid,text,text,jsonb) from public,anon;
grant execute on function public.beauty_import_clients(uuid,uuid,text,text,jsonb) to authenticated,service_role;

revoke all on function private.beauty_list_import_jobs(uuid,uuid) from public,anon;
grant execute on function private.beauty_list_import_jobs(uuid,uuid) to authenticated,service_role;
revoke all on function public.beauty_list_import_jobs(uuid,uuid) from public,anon;
grant execute on function public.beauty_list_import_jobs(uuid,uuid) to authenticated,service_role;
