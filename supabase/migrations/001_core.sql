-- NCR Suite V1.0.1 — socle multi-entreprises sécurisé
...
create table if not exists public.organizations (
  ...
  primary_color text not null default '#2997ff' check (primary_color ~ '^#[0-9A-Fa-f]{6}$'),
  logo_url text,
...
begin
  ...
  values (v_name, v_slug, p_business_type, p_primary_color, auth.uid())
...
revoke all on function public.update_organization_branding(uuid,text,text) from public;
...
drop policy if exists "modules_member_select" on public.organization_modules;
...
-- Les modifications sensibles (plan, statut, type métier) restent réservées au backend NCR.
-- Le nom et la couleur sont modifiés uniquement via update_organization_branding().
