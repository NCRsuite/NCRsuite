-- NCR Suite V2.21.2 - Validation production finale
-- A executer apres 083_training_data_recovery.sql.
...
begin
  ...
  from unnest(array[
    'supabase','cloudflare','pwa','roles','domains','formation','backup'
  ]::text[]) expected(key)
  ...
  from (
    values
      (1,'supabase','Migration 084 executee et version 2.21.2 confirmee'),
      ...
      (7,'backup','Sauvegarde Supabase et procedure de retour arriere confirmees')
  ) manual(position,key,label);

  v_checks:=jsonb_build_array(
    jsonb_build_object(
      'key','database_release','category','Version','label','Version Supabase',
      'status',case when v_release.database_version='2.21.2'
                        and v_release.expected_frontend_version='2.21.2'
                        and v_release.expected_pwa_cache='ncr-suite-shell-v2.21.2-final-production-validation'
      ...
      'action','Executer integralement la migration 084.'
    ),
    jsonb_build_object(
  ...
  if coalesce(p_store,false) then
    v_checks:=v_checks||jsonb_build_array(jsonb_build_object(
      ...
      'detail',v_manual_completed||' contrôle(s) confirmé(s) sur 7.',
...
insert into public.platform_release_state(
  ...
  true,'2.21.2','2.21.2','ncr-suite-shell-v2.21.2-final-production-validation',
  now(),auth.uid(),
  'V2.21.2 : validation production finale, controles automatiques transversaux, historique et export super administrateur.'
