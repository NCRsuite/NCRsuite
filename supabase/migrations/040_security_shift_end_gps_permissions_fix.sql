...
-- V2.6.3 — Une vacation déjà prise peut toujours être terminée par son agent,
-- même lorsque la fenêtre horaire habituelle est dépassée.
-- La main courante est rouverte techniquement le temps d'ajouter la fin de poste,
-- puis immédiatement clôturée à nouveau.
...
begin
  ...
  -- La fenêtre horaire limite uniquement une prise de poste tardive ou anticipée.
  -- Une vacation déjà prise doit pouvoir être terminée, même longtemps après l'heure prévue.
  if v_action = 'start'
     and not v_manager
     and not p_force
     and (now() < v_shift.starts_at - interval '4 hours' or now() > v_shift.ends_at + interval '8 hours') then
  ...
  if v_action = 'start' then
    ...
    if v_shift.clocked_in_at is null then
      ...
      -- Le QG peut régulariser une prise manquante.
      if v_shift.logbook_status = 'closed' then
        update public.security_shifts
    ...
    -- Une ancienne fermeture partielle ne doit pas empêcher la vraie fin de poste.
    ...
    set status = 'completed',
        ...
        logbook_closed_at = v_event_time,
        logbook_closed_by = auth.uid(),
        logbook_closed_source = v_source,
        updated_at = now()
    where organization_id = p_organization_id and id = p_shift_id
    returning * into v_shift;
  end if;
