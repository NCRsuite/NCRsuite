-- Catégories de prestations Beauty, propres à chaque enseigne via services.company_id.
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'services_category_name_length_check'
  ) then
    alter table public.services
      add constraint services_category_name_length_check
      check (category_name is null or char_length(trim(category_name)) between 2 and 80);
  end if;
end $$;

create index if not exists services_company_category_idx
  on public.services (organization_id, company_id, category_name)
  where active = true;
