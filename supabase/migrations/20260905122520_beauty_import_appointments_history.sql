...
CREATE OR REPLACE FUNCTION public.beauty_import_appointments(p_organization_id uuid, p_company_id uuid, p_source_provider text, p_file_name text, p_rows jsonb)
 RETURNS jsonb
 LANGUAGE sql
 SET search_path = public,private,pg_catalog
AS $function$
  select private.beauty_import_appointments(p_organization_id,p_company_id,p_source_provider,p_file_name,p_rows);
$function$;

CREATE OR REPLACE FUNCTION public.beauty_preview_appointment_import(p_organization_id uuid, p_company_id uuid, p_source_provider text, p_rows jsonb)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path = public,private,pg_catalog
AS $function$
  select private.beauty_preview_appointment_import(p_organization_id,p_company_id,p_source_provider,p_rows);
$function$;

revoke all on function private.beauty_preview_appointment_import(uuid,uuid,text,jsonb) from public,anon;
grant execute on function private.beauty_preview_appointment_import(uuid,uuid,text,jsonb) to authenticated,service_role;
revoke all on function public.beauty_preview_appointment_import(uuid,uuid,text,jsonb) from public,anon;
grant execute on function public.beauty_preview_appointment_import(uuid,uuid,text,jsonb) to authenticated,service_role;

revoke all on function private.beauty_import_appointments(uuid,uuid,text,text,jsonb) from public,anon;
grant execute on function private.beauty_import_appointments(uuid,uuid,text,text,jsonb) to authenticated,service_role;
revoke all on function public.beauty_import_appointments(uuid,uuid,text,text,jsonb) from public,anon;
grant execute on function public.beauty_import_appointments(uuid,uuid,text,text,jsonb) to authenticated,service_role;
