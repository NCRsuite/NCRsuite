...
import { formatCleaningDateTime, formatCleaningMoney, nullableCleaningText, type CleaningAgentRecord, type CleaningInterventionRecord, type CleaningSiteRecord } from '../features/cleaning/types';
...
export function CleaningPlanningPage() {
  ...
  const [rows, setRows] = useState<CleaningInterventionRecord[]>([]); const [sites, setSites] = useState<CleaningSiteRecord[]>([]); const [agents, setAgents] = useState<CleaningAgentRecord[]>([]);
  const [formOpen, setFormOpen] = useState(false); const [form, setForm] = useState({ siteId: '', agentId: '', title: 'Entretien régulier', startsAt: defaultStart(), endsAt: defaultEnd(), breakMinutes: '0', notes: '' });
  ...
  async function load() {
    ...
    const [interventionResult, siteResult, agentResult] = await Promise.all([
    ...
    const firstError = interventionResult.error || siteResult.error || agentResult.error; if (firstError) setError(firstError.message); else { setRows((interventionResult.data ?? []) as CleaningInterventionRecord[]); setSites((siteResult.data ?? []) as CleaningSiteRecord[]); setAgents((agentResult.data ?? []) as CleaningAgentRecord[]); }
    setLoading(false);
  }
  ...
  async function createIntervention(event: FormEvent) {
    ...
    const payload = { organization_id: organization.id, site_id: form.siteId, agent_id: form.agentId, title: form.title.trim() || 'Intervention de nettoyage', starts_at: start.toISOString(), ends_at: end.toISOString(), break_minutes: Math.max(0, Number(form.breakMinutes) || 0), planned_price_cents: plannedPrice, notes: nullableCleaningText(form.notes), created_by: user.id };
    try {
      ...
      setRows((current) => [...current, created].sort((a, b) => a.starts_at.localeCompare(b.starts_at))); setForm({ siteId: '', agentId: '', title: 'Entretien régulier', startsAt: defaultStart(), endsAt: defaultEnd(), breakMinutes: '0', notes: '' }); setFormOpen(false); setSuccess('L’intervention a été planifiée.');
