  ...
  const load = useCallback(async () => {
    ...
    if (demoMode || !supabase) {
      ...
      setSummary({
        ...
        active_members: isFieldBusiness ? 0 : 1,
        pending_invitations: 0,
        available_seats: hasTeamAccess ? Math.max(0, planDefinition.memberLimit - (isFieldBusiness ? 0 : 1)) : 0,
        ...
        <>
          ...
          {!summary.invitations_enabled ? (
            <section className="panel upgrade-panel">
              ...
              <div>
                <p className="eyebrow">{isTraining ? 'OFFRE PROFESSIONNELLE' : isSecurity ? 'MODULE OU OFFRE ESSENTIELLE' : 'OFFRE ESSENTIELLE'}</p>
                <h2>Donnez un accès personnel à vos collaborateurs</h2>
                <p>{isTraining ? 'L’offre Professionnelle permet de créer des accès employés distincts et de choisir leur rôle dans l’espace Formation.' : isSecurity ? 'La formule Découverte peut recevoir le module Accès Agent Terrain pour 5 agents, puis une extension jusqu’à 10. Les rondes QR, la main courante et les consignes peuvent également être ajoutées séparément.' : isCleaning ? 'La formule Découverte reste réservée au gestionnaire. L’offre Essentielle permet de connecter jusqu’à 10 agents à leur planning, au pointage, aux consignes et aux rapports de passage.' : isRestaurant ? 'La formule Découverte reste réservée au gestionnaire. L’offre Essentielle permet de connecter jusqu’à 10 employés au planning, aux réservations, à la carte et aux outils d’hygiène.' : 'La formule Découverte reste limitée au compte propriétaire. À partir de l’offre Essentielle, vous pouvez inviter jusqu’à 2 collaborateurs supplémentaires, chacun avec son propre planning.'}</p>
              </div>
              <span className="plan-lock-badge">{isTraining ? 'Professionnelle' : isSecurity ? 'Module dès 12,90 € · ou Essentielle 69,90 €' : isCleaning ? 'Essentielle · 49,90 € HT / mois' : isRestaurant ? 'Essentielle · 49,90 € HT / mois' : 'Disponible à partir de 19,90 € HT / mois'}</span>
            </section>
          ) : !canView ? (
