...
export function RestaurantDashboardPage() {
  ...
  return <div className="page restaurant-page restaurant-dashboard-page"><header className="page-header"><div><p className="eyebrow">RESTAURATION</p><h1>Pilotage du restaurant</h1><p>Réservations, équipe, carte, hygiène et stocks en un seul regard.</p></div><div className="header-actions"><Link className="primary-button" to="/reservations?new=1"><Icon name="calendar" size={18}/>Nouvelle réservation</Link></div></header>
    ...
    <section className="stats-grid"><StatCard label="Réservations aujourd’hui" value={String(todayReservations.length)} detail={`${covers} couverts prévus`} icon="calendar" loading={loading}/><StatCard label="Employés planifiés" value={String(new Set(todayShifts.map((row) => row.employee_id)).size)} detail={`${todayShifts.length} service${todayShifts.length > 1 ? 's' : ''}`} icon="users" loading={loading}/><StatCard label="Stocks sous seuil" value={String(lowStock.length)} detail="produits à commander" icon="briefcase" loading={loading}/><StatCard label="Alertes hygiène" value={organizationHasFeature(organization, 'restaurant_temperatures') ? String(nonCompliant.length) : '—'} detail={organizationHasFeature(organization, 'restaurant_temperatures') ? 'relevés non conformes' : 'offre Essentielle'} icon="alert" loading={loading}/></section>
    <section className="restaurant-dashboard-grid"><article className="panel restaurant-dashboard-main"><div className="panel-header"><div><p className="eyebrow">PROCHAINEMENT</p><h2>Réservations du jour</h2></div><Link className="secondary-button compact-button" to="/reservations">Tout voir</Link></div>{loading ? <PremiumSkeleton rows={4} label="Chargement des réservations du jour"/> : nextReservations.length === 0 ? <div className="restaurant-empty"><Icon name="calendar" size={30}/><strong>Aucune réservation à venir</strong></div> : <div className="restaurant-card-list">{nextReservations.map((row) => <article className="restaurant-record-card" key={row.id}><span className="restaurant-record-icon"><Icon name="utensils" size={20}/></span><div className="restaurant-record-main"><strong>{row.guest_name} · {row.party_size} pers.</strong><span>{formatRestaurantDateTime(row.reservation_at)}{row.restaurant_tables?.name ? ` · ${row.restaurant_tables.name}` : ''}</span><small>{row.guest_phone || row.guest_email || 'Coordonnées non renseignées'}</small></div><span className={`restaurant-status-pill ${row.status}`}>{row.status === 'confirmed' ? 'Confirmée' : row.status === 'seated' ? 'Installée' : row.status === 'completed' ? 'Terminée' : 'En attente'}</span></article>)}</div>}</article>
