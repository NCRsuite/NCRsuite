...
type PortalPermissions = { planning: boolean; reports: boolean; anomalies: boolean; quality: boolean; documents: boolean; messages: boolean };
...
type PortalMessage = { id: string; author_type: 'cleaning' | 'client'; author_name: string | null; body: string; read_by_client_at: string | null; read_by_cleaning_at: string | null; created_at: string };
type PortalDocument = { id: string; client_id: string; site_id: string | null; intervention_id: string | null; title: string; category: string; storage_path: string; mime_type: string | null; size_bytes: number | null; published_at: string; cleaning_sites?: { name: string } | null };
type SiteOption = { id: string; name: string };

const defaultPermissions: PortalPermissions = { planning: true, reports: true, anomalies: true, quality: true, documents: true, messages: true };
const permissionLabels: Array<[keyof PortalPermissions, string]> = [
  ['planning', 'Interventions et planning'], ['reports', 'Rapports de passage'], ['anomalies', 'Anomalies'], ['quality', 'Contrôles qualité'], ['documents', 'Documents'], ['messages', 'Messagerie']
];
...
export function CleaningClientPortalAdminPage() {
  ...
  const loadOverview = useCallback(async () => {
    ...
    const { data, error: rpcError } = await supabase.rpc('cleaning_client_portal_admin_overview', { p_organization_id: organization.id });
  ...
  const loadClientData = useCallback(async (clientId: string) => {
    ...
    const [messageResult, documentResult, siteResult] = await Promise.all([
      supabase.rpc('cleaning_client_portal_admin_messages', { p_organization_id: organization.id, p_client_id: clientId }),
      supabase.from('cleaning_client_portal_documents').select('id,client_id,site_id,intervention_id,title,category,storage_path,mime_type,size_bytes,published_at,cleaning_sites(name)').eq('organization_id', organization.id).eq('client_id', clientId).eq('status', 'active').order('published_at', { ascending: false }),
      supabase.from('cleaning_sites').select('id,name').eq('organization_id', organization.id).eq('client_id', clientId).eq('status', 'active').order('name')
    ]);
    ...
    if (!messageResult.error) {
      setClients((current) => current.map((client) => client.client_id === clientId ? { ...client, unread_messages: 0 } : client));
    }
  }, [organization?.id]);
  ...
  async function invite(event: FormEvent) {
    ...
    const { error: inviteError } = await supabase.rpc('create_cleaning_client_portal_invitation', {
  ...
  async function invitationAction(invitation: PortalInvitation, action: 'resend' | 'revoke') {
    ...
    const result = action === 'resend'
      ? await supabase.rpc('resend_cleaning_client_portal_invitation', { p_organization_id: organization.id, p_invitation_id: invitation.id })
      : await supabase.rpc('revoke_cleaning_client_portal_invitation', { p_organization_id: organization.id, p_invitation_id: invitation.id });
  ...
  async function toggleAccount(account: PortalAccount) {
    ...
    const { error: updateError } = await supabase.rpc('set_cleaning_client_portal_account', {
  ...
  async function sendMessage(event: FormEvent) {
    ...
    const { error: sendError } = await supabase.rpc('cleaning_client_portal_admin_send_message', { p_organization_id: organization.id, p_client_id: selected.client_id, p_body: messageBody.trim() });
  ...
  async function uploadDocument(event: FormEvent) {
    ...
    try {
      const { error: uploadError } = await supabase.storage.from('cleaning-client-documents').upload(path, docFile, { contentType: mimeType, upsert: false });
      if (uploadError) throw uploadError;
      const { error: insertError } = await supabase.from('cleaning_client_portal_documents').insert({
      ...
      if (insertError) { await supabase.storage.from('cleaning-client-documents').remove([path]); throw insertError; }
  ...
  async function deleteDocument(document: PortalDocument) {
    ...
    const { error: storageError } = await supabase.storage.from('cleaning-client-documents').remove([document.storage_path]);
    if (storageError) { setError(storageError.message); setBusy(''); return; }
    const { error: deleteError } = await supabase.from('cleaning_client_portal_documents').delete().eq('organization_id', organization.id).eq('id', document.id);
  ...
  return <div className="page cleaning-page client-portal-admin-page">
    <header className="page-header"><div><p className="eyebrow">PHASE 3 · NETTOYAGE</p><h1>Portail clients</h1><p>Partage les interventions, rapports, anomalies, contrôles qualité et documents avec chaque client dans un espace sécurisé.</p></div><button className="secondary-button" onClick={() => void loadOverview()}><Icon name="activity" size={18}/>Actualiser</button></header>
    ...
    <section className="client-portal-admin-layout">
      <aside className="panel client-portal-client-list">
        <div className="panel-header"><div><p className="eyebrow">CLIENTS</p><h2>{clients.length} client{clients.length > 1 ? 's' : ''}</h2></div></div>
        {loading ? <div className="security-empty">Chargement…</div> : clients.length === 0 ? <div className="security-empty"><Icon name="building" size={30}/><strong>Aucun client actif</strong><span>Crée d’abord une fiche client Nettoyage.</span></div> : clients.map((client) => <button key={client.client_id} className={`client-portal-client-button ${selected?.client_id === client.client_id ? 'active' : ''}`} onClick={() => setSelectedId(client.client_id)}>
