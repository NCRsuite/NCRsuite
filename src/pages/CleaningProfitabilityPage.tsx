...
export function CleaningProfitabilityPage() {
  ...
  return <div className="page cleaning-page cleaning-profitability-page">
    ...
    <section className="stats-grid"><StatCard label="Chiffre prévisionnel" value={formatCleaningMoney(totals.revenue)} detail="interventions non annulées" icon="creditCard" loading={loading}/><StatCard label="Coût main-d’œuvre" value={formatCleaningMoney(totals.labor)} detail="selon le coût horaire renseigné" icon="users" loading={loading}/><StatCard label="Produits consommés" value={formatCleaningMoney(totals.consumables)} detail="saisi par intervention" icon="briefcase" loading={loading}/><StatCard label="Marge estimée" value={formatCleaningMoney(totals.margin)} detail={`${totalMarginRate.toLocaleString('fr-FR')} % du chiffre`} icon="chart" loading={loading}/></section>
