...
export function RestaurantKitchenPage() {
  ...
  return <div className="page restaurant-page restaurant-kitchen-page restaurant-premium-workspace">
    ...
    {loading ? <div className="panel restaurant-empty">Chargement…</div> : grouped.length === 0 ? <div className="panel restaurant-empty restaurant-kitchen-empty"><span className="restaurant-empty-illustration">👨‍🍳</span><strong>Aucune préparation en attente</strong><span>La cuisine est à jour.</span></div> : <section className="restaurant-kitchen-grid">{grouped.map(({ order, items: orderItems }) => {
      ...
      return <article className={`panel restaurant-kitchen-ticket ${ticketStatus}`} key={order.id}><header><div className="restaurant-kitchen-ticket-identity"><span className="restaurant-kitchen-ticket-number">#{order.order_number}</span><div><p className="eyebrow">COMMANDE</p><h2>{order.restaurant_tables?.name || 'Commande libre'}</h2></div></div><div className="restaurant-kitchen-ticket-time"><strong>{elapsedLabel(oldestSentAt)}</strong><span>{new Intl.DateTimeFormat('fr-FR', { hour: '2-digit', minute: '2-digit' }).format(new Date(order.opened_at))}</span></div></header><div className="restaurant-kitchen-ticket-progress"><span className={ticketStatus}/></div><div className="restaurant-kitchen-items">{orderItems.map((item) => <div key={item.id} className={`restaurant-kitchen-item ${item.status}`}><div className="restaurant-kitchen-item-copy"><div className="restaurant-kitchen-item-title"><span className="restaurant-kitchen-quantity">{item.quantity}×</span><strong>{item.item_name}</strong>{item.menu_item_id && <Link className="restaurant-kitchen-recipe-link" to={`/recettes?plat=${item.menu_item_id}`}>Recette</Link>}</div>{item.notes && <p><span>!</span>{item.notes}</p>}<small><span>{stationIcons[item.station]}</span>{stationLabels[item.station]} · {item.status === 'sent' ? 'À préparer' : item.status === 'in_progress' ? 'En préparation' : 'Prêt à servir'}</small></div><div className="restaurant-kitchen-item-action">{item.status === 'sent' && <button type="button" className="restaurant-kitchen-action start" onClick={() => void changeStatus(item, 'in_progress')}>Commencer</button>}{item.status === 'in_progress' && <button type="button" className="restaurant-kitchen-action ready" onClick={() => void changeStatus(item, 'ready')}>Marquer prêt</button>}{item.status === 'ready' && <button type="button" className="restaurant-kitchen-action serve" onClick={() => void changeStatus(item, 'served')}>Servi ✓</button>}</div></div>)}</div></article>;
    })}</section>}
  </div>;
}
