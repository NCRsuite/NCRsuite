...
export function SecurityBillingPage() {
      ...
      try {
        if (demoMode || !supabase) {
          ...
          const lines = relevantSites.map((site) => {
            ...
            const minutes = rows.reduce((sum, shift) => sum + (mode === 'invoice' ? (shift.actual_minutes ?? securityShiftMinutes(shift)) : securityShiftMinutes(shift)), 0);
        ...
        if (active) {
          ...
          setActualMinutes(Object.fromEntries(shifts.map((shift) => [shift.id, String(shift.actual_minutes ?? securityShiftMinutes(shift))])));
        }
  ...
  async function saveActual(shift: CompletedShift) {
    ...
    if (!Number.isInteger(minutes) || minutes < 0 || minutes > 2880) { setError('La durée réalisée doit être exprimée en minutes valides.'); return; }
    setSavingShiftId(shift.id); setError(''); setSuccess('');
    try {
      if (demoMode || !supabase) {
        const all = demoShifts.map((row) => row.id === shift.id ? { ...row, actual_minutes: minutes } : row);
        localStorage.setItem(`ncr-suite-security-shifts-${organization.id}`, JSON.stringify(all)); setDemoShifts(all);
      } else {
        const { error: rpcError } = await supabase.rpc('validate_security_shift_actual_minutes', { p_organization_id: organization.id, p_shift_id: shift.id, p_actual_minutes: minutes, p_note: null });
        if (rpcError) throw rpcError;
      }
      setSuccess('La durée réalisée de la vacation a été validée.'); setPreviewNonce((value) => value + 1);
...
${organization?.public_name || organization?.name || ''}`);
  ...
  return <div className="page security-page security-billing-page">
    ...
    <section className="security-billing-grid"><article className="panel security-billing-builder"><div className="panel-header"><div><p className="eyebrow">{mode === 'invoice' ? 'NOUVELLE FACTURE' : 'NOUVELLE PRÉFACTURE'}</p><h2>{mode === 'invoice' ? 'Vacations réalisées' : 'Heures programmées'}</h2><p>{mode === 'invoice' ? 'Seules les missions marquées Réalisées et encore non facturées sont intégrées.' : 'Le calcul reprend les heures programmées et le tarif de chaque site.'}</p></div></div>
      ...
      <aside className="panel security-billing-rule"><span><Icon name={mode === 'invoice' ? 'check' : 'shield'} size={24}/></span><p className="eyebrow">{mode === 'invoice' ? 'FACTURE DÉFINITIVE' : 'RÈGLE DÉCOUVERTE'}</p><h2>{mode === 'invoice' ? 'Après réalisation' : 'Calcul prévisionnel'}</h2><p>{mode === 'invoice' ? 'Une facture numérotée est émise depuis les vacations réalisées. Chaque mission ne peut être facturée qu’une fois.' : 'Pour chaque site : heures programmées × tarif horaire du site.'}</p>{mode === 'invoice' && <p><strong>TVA configurée : {organization.security_default_vat_rate ?? 20} %</strong><br/>Échéance : {organization.security_payment_terms_days ?? 30} jours.</p>}</aside>
    </section>
