...
export function CleaningClientPortalInvitationPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error: rpcError } = await supabase.rpc('get_cleaning_client_portal_invitation', { p_token: token });
      if (!active) return;
  ...
  async function acceptInvitation() {
    ...
    setError('');
    try {
      const { error: acceptError } = await supabase.rpc('accept_cleaning_client_portal_invitation', { p_token: token });
      if (acceptError) throw acceptError;
      navigate('/espace-client-nettoyage', { replace: true });
    } catch (caught) {
  ...
  return <div className="security-client-public-shell" style={{ '--portal-accent': accent } as React.CSSProperties}>
    ...
    <section className="security-client-invitation-card">
      <div className="security-client-public-brand">
        {details?.organization_logo_url ? <img src={details.organization_logo_url} alt="" /> : <span><Icon name="sparkles" size={26}/></span>}
        <div><strong>{details?.organization_name || 'Portail client Nettoyage'}</strong><small>Propulsé par NCR Suite</small></div>
      </div>
      ...
      {loading ? <div className="security-client-loading"><span/><p>Vérification de l’invitation…</p></div> : details ? <>
        ...
        {!available ? <div className="security-client-accept-box">
          ...
          {details.invitation_status === 'accepted' && <Link className="primary-button full" to="/espace-client-nettoyage">Ouvrir mon portail client</Link>}
      ...
      <div className="security-client-public-footer"><Link to="/espace-client-nettoyage">Déjà client ? Ouvrir le portail</Link><span>·</span><a href="mailto:contact@ncr-suite.fr">Assistance NCR Suite</a></div>
    </section>
  </div>;
}
