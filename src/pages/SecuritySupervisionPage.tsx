...
import {
  ...
  type SecurityAgentPresenceRecord,
...
function presenceLabel(presence: SecurityAgentPresenceRecord | undefined, now: number) {
  if (!presence) return { text: 'Mode vacation absent', tone: 'inactive' };
  const age = now - new Date(presence.last_seen_at).getTime();
  if (presence.status === 'stopped') return { text: 'Mode vacation arrêté', tone: 'inactive' };
  if (age > 180000) return { text: 'Connexion perdue', tone: 'danger' };
  if (presence.network_status === 'offline') return { text: 'Hors connexion', tone: 'warning' };
  if (presence.app_state === 'hidden') return { text: 'Application en arrière-plan', tone: 'warning' };
  if (!presence.tracking_active) return { text: 'GPS interrompu', tone: 'warning' };
  return { text: 'Agent connecté', tone: 'active' };
}
...
export function SecuritySupervisionPage() {
  ...
  const [presenceRows, setPresenceRows] = useState<SecurityAgentPresenceRecord[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const [shiftResult, positionResult, presenceResult, ptiResult, alertResult] = await Promise.all([
      ...
      supabase.from('security_agent_presence')
        .select('id,organization_id,agent_id,shift_id,status,network_status,app_state,tracking_active,wake_lock_active,started_at,last_seen_at,stopped_at,created_at,updated_at')
        .eq('organization_id', organization.id).order('last_seen_at',{ascending:false}).limit(500),
    ...
    const firstError = shiftResult.error || positionResult.error || presenceResult.error || ptiResult.error || alertResult.error;
    if (firstError) setError(firstError.message);
      ...
      setPresenceRows((presenceResult.data ?? []) as unknown as SecurityAgentPresenceRecord[]);
  ...
  useEffect(() => {
    ...
    const channel = client.channel(`security-supervision-${organization.id}`)
      ...
      .on('postgres_changes',{event:'*',schema:'public',table:'security_agent_presence',filter:`organization_id=eq.${organization.id}`},()=>void load())
  ...
  const latestPresenceByAgent = useMemo(() => {
    const map = new Map<string, SecurityAgentPresenceRecord>();
    for (const row of presenceRows) if (!map.has(row.agent_id)) map.set(row.agent_id,row);
    return map;
  }, [presenceRows]);
  ...
  const connectedAgents = activeShifts.filter((row) => presenceLabel(latestPresenceByAgent.get(row.agent_id), now).tone === 'active').length;
  const lostAgents = activeShifts.filter((row) => presenceLabel(latestPresenceByAgent.get(row.agent_id), now).tone === 'danger').length;
  ...
  return <div className="page security-page security-supervision-page">
    <header className="page-header"><div><p className="eyebrow">SÉCURITÉ PROFESSIONNELLE</p><h1>Supervision temps réel</h1><p>Vacations, présence applicative, positions GPS, PTI et alertes SOS sur un seul écran.</p></div><div className="security-header-actions"><Link className="secondary-button" to="/geolocalisation"><Icon name="map" size={18}/>Géolocalisation</Link><button className="primary-button" type="button" onClick={()=>void load()} disabled={loading}><Icon name="activity" size={18}/>{loading?'Actualisation…':'Actualiser'}</button></div></header>
    ...
    {lostAgents>0 && <div className="security-callout warning"><Icon name="activity" size={22}/><div><strong>{lostAgents} agent{lostAgents>1?'s':''} sans battement récent</strong><span>L’application peut être fermée, suspendue ou sans réseau. Vérifie la vacation et contacte l’agent si nécessaire.</span></div></div>}

    <section className="stats-grid">
      <article className="stat-card"><span><Icon name="activity" size={22}/></span><div><small>Vacations en cours</small><strong>{loading?'…':activeShifts.length}</strong><p>{upcomingShifts.length} à venir</p></div></article>
      <article className="stat-card"><span><Icon name="users" size={22}/></span><div><small>Agents connectés</small><strong>{loading?'…':connectedAgents}</strong><p>{lostAgents} connexion(s) perdue(s)</p></div></article>
      <article className="stat-card"><span><Icon name="map" size={22}/></span><div><small>Agents localisés</small><strong>{loading?'…':[...latestPositionByAgent.values()].filter((row)=>now-new Date(row.recorded_at).getTime()<5*60000).length}</strong><p>position depuis moins de 5 min</p></div></article>
      <article className="stat-card"><span><Icon name="shield" size={22}/></span><div><small>PTI actifs</small><strong>{loading?'…':ptiSessions.filter((row)=>row.status==='active').length}</strong><p>{ptiSessions.filter((row)=>row.status==='alerted').length} en alerte</p></div></article>
      <article className="stat-card"><span><Icon name="alert" size={22}/></span><div><small>Urgences ouvertes</small><strong>{loading?'…':criticalAlerts.length}</strong><p>{alerts.filter((row)=>row.status==='acknowledged').length} prises en charge</p></div></article>
    </section>
    ...
    <section className="dashboard-grid security-supervision-grid"><article className="panel large-panel"><div className="panel-header"><div><p className="eyebrow">DISPOSITIF EN COURS</p><h2>Agents actuellement en vacation</h2></div></div>{loading?<div className="security-empty">Chargement du dispositif…</div>:activeShifts.length===0?<div className="security-empty"><Icon name="calendar" size={31}/><strong>Aucune vacation en cours</strong><span>Les prochaines missions apparaîtront automatiquement.</span></div>:<div className="security-supervision-shifts">{activeShifts.map((shift)=>{
      const position=latestPositionByAgent.get(shift.agent_id);
      const presence=latestPresenceByAgent.get(shift.agent_id);
      const pti=ptiByAgent.get(shift.agent_id);
      const positionLive=position&&now-new Date(position.recorded_at).getTime()<5*60000;
      const presenceState=presenceLabel(presence,now);
      return <article key={shift.id}><span className="security-record-icon" style={{background:`${shift.security_sites?.color_hex||'#0A84FF'}22`,color:shift.security_sites?.color_hex||'#0A84FF'}}><Icon name="shield" size={19}/></span><div><strong>{shift.security_agents?securityPersonName(shift.security_agents.first_name,shift.security_agents.last_name):'Agent'}</strong><span>{shift.security_sites?.name||'Site'} · {shift.title||'Mission de sécurité'}</span><small>{formatSecurityDateTime(shift.starts_at)} → {new Intl.DateTimeFormat('fr-FR',{hour:'2-digit',minute:'2-digit'}).format(new Date(shift.ends_at))}</small>{presence&&<small>Dernier battement {relativeAge(presence.last_seen_at,now)} · {presence.wake_lock_active?'écran maintenu':'écran non maintenu'}</small>}{position&&<small>Dernière position {relativeAge(position.recorded_at,now)}</small>}</div><div className="security-supervision-statuses"><span className={`status-chip ${presenceState.tone}`}>{presenceState.text}</span><span className={`status-chip ${positionLive?'active':'inactive'}`}>{positionLive?'GPS récent':'GPS ancien/absent'}</span><span className={`status-chip ${pti?.status==='active'?'active':pti?.status==='alerted'?'danger':'inactive'}`}>{pti?.status==='active'?'PTI actif':pti?.status==='alerted'?'PTI alerte':'PTI inactif'}</span></div></article>})}</div>}</article>
