  ...
  async function validateSession(session: TrainingSessionRecord) {
    ...
    try {
      if (demoMode || !supabase) {
        ...
        setSuccess(`Session validée. ${queued} convocation${queued > 1 ? 's' : ''} et ${initialQueued} évaluation${initialQueued > 1 ? 's initiales' : ' initiale'} ont été programmées.`);
        await loadData();
      }
    ...
    <section className="training-workflow-layout">
      ...
      <main className="panel training-workflow-cockpit">
        {!selectedSession || !sessionData ? <div className="training-empty"><Icon name="clipboard" size={31} /><strong>Sélectionne une session</strong><span>Le cockpit affichera le parcours complet et la prochaine action.</span></div> : <>
          ...
          {selectedSession.status !== 'draft' && selectedSession.status !== 'completed' && sessionData.isEnded && <section className="training-workflow-next-action closure"><span><Icon name="check" size={21} /></span><div><p className="eyebrow">SESSION ARRIVÉE À SON TERME</p><h3>Terminer et lancer la clôture automatisée</h3><p>Les évaluations finales sont envoyées automatiquement. Les attestations sont ensuite générées selon vos réglages et le dossier se finalise automatiquement.</p></div><button type="button" className="primary-button" disabled={busyId === `finish-${selectedSession.id}`} onClick={() => void finishSession(selectedSession)}>{busyId === `finish-${selectedSession.id}` ? 'Clôture…' : 'Terminer la session'}</button></section>}
          {selectedSession.status === 'completed' && !sessionData.isFinalized && <section className="training-workflow-next-action automation"><span><Icon name="refresh" size={21} /></span><div><p className="eyebrow">CLÔTURE AUTOMATISÉE</p><h3>Le dossier se complète sans ressaisie</h3><p>Les évaluations finales, les relances et l’envoi des attestations sont suivis automatiquement. Ouvrez le dossier complet pour voir les éléments encore attendus.</p></div><Link className="primary-button" to={`/dossiers-formation?session=${selectedSession.id}`}>Suivre la clôture</Link></section>}
