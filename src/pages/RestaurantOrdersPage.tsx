...
export function RestaurantOrdersPage() {
  ...
  return <div className="page restaurant-page restaurant-orders-page">
    <header className="page-header"><div><p className="eyebrow">RESTAURATION · SERVICE</p><h1>Prise de commande tactile</h1><p>Appuie sur les produits : ils sont ajoutés immédiatement à la note. Aucun moyen de paiement n’est enregistré.</p></div></header>
    ...
    <section className="restaurant-order-layout">
      <aside className="panel restaurant-order-tables">
        <div className="panel-header"><div><p className="eyebrow">TABLES</p><h2>Service en cours</h2></div></div>
        <div className="restaurant-order-table-list">
          <button type="button" className={!selectedTableId ? 'active' : ''} onClick={() => { setSelectedTableId(''); setSelectedOrderId(orders.find((row) => !row.table_id)?.id ?? ''); setSearchParams({}); }}><Icon name="utensils" size={18}/><span><strong>Note libre</strong><small>Comptoir ou à emporter</small></span></button>
          {tables.map((table) => { const active = activeOrderByTable.get(table.id); return <button type="button" key={table.id} className={selectedTableId === table.id ? 'active' : ''} onClick={() => { setSelectedTableId(table.id); setSelectedOrderId(active?.id ?? ''); setSearchParams({ table: table.id }); }}><span className={`restaurant-order-table-dot ${active ? 'busy' : ''}`}/><span><strong>{table.name}</strong><small>{active ? `${formatRestaurantMoney(active.total_cents)} · note ouverte` : `${table.capacity} places · libre`}</small></span></button>; })}
        </div>
        {!selectedOrder && <div className="restaurant-order-open-box"><label>Nombre de couverts<input type="number" min="1" max="100" value={guestCount} onChange={(event) => setGuestCount(event.target.value)}/></label><button type="button" className="primary-button" disabled={saving || (Boolean(selectedTableId) && !selectedTable)} onClick={() => void openOrder()}><Icon name="plus" size={17}/>Ouvrir la note</button></div>}
      </aside>

      <main className="panel restaurant-order-menu">
        <div className="restaurant-order-menu-toolbar"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un plat…"/><div className="restaurant-order-category-tabs"><button type="button" className={categoryId === 'all' ? 'active' : ''} onClick={() => setCategoryId('all')}>Tout</button>{categories.map((category) => <button type="button" key={category.id} className={categoryId === category.id ? 'active' : ''} onClick={() => setCategoryId(category.id)}>{category.name}</button>)}</div></div>
        {loading ? <div className="restaurant-empty">Chargement…</div> : !selectedOrder ? <div className="restaurant-empty"><Icon name="clipboard" size={34}/><strong>Sélectionne une table et ouvre sa note</strong><span>Les produits seront ensuite ajoutés en un seul appui.</span></div> : visibleMenuItems.length === 0 ? <div className="restaurant-empty">Aucun produit disponible dans cette catégorie.</div> : <div className="restaurant-order-product-grid">{visibleMenuItems.map((menuItem) => <button type="button" key={menuItem.id} disabled={saving || selectedOrder.status === 'bill_requested'} onClick={() => void addMenuItem(menuItem)}><span>{menuItem.name}</span><strong>{formatRestaurantMoney(menuItem.price_cents)}</strong>{menuItem.allergens.length > 0 && <small>{menuItem.allergens.slice(0, 2).join(' · ')}</small>}</button>)}</div>}
      </main>

      <aside className="panel restaurant-order-note">
        <div className="panel-header"><div><p className="eyebrow">NOTE PROVISOIRE</p><h2>{selectedOrder?.restaurant_tables?.name || (selectedOrder ? `Commande n°${selectedOrder.order_number}` : 'Aucune note')}</h2></div>{selectedOrder && <span className={`restaurant-status-pill ${selectedOrder.status}`}>{selectedOrder.status === 'bill_requested' ? 'À encaisser' : selectedOrder.status}</span>}</div>
        {!selectedOrder ? <div className="restaurant-empty">Ouvre une note pour commencer.</div> : <>
          <div className="restaurant-order-line-list">{selectedItems.length === 0 ? <div className="restaurant-empty">La note est vide.</div> : selectedItems.map((item) => <article key={item.id} className={`restaurant-order-line ${item.status}`}><div className="restaurant-order-line-main"><strong>{item.item_name}</strong><span>{courseLabels[item.course]} · {formatRestaurantMoney(item.unit_price_cents)}</span>{item.notes && <small>{item.notes}</small>}</div><div className="restaurant-order-quantity"><button type="button" disabled={item.status !== 'draft' || item.quantity <= 1} onClick={() => void updateItem(item, { quantity: item.quantity - 1 })}>−</button><b>{item.quantity}</b><button type="button" disabled={item.status !== 'draft'} onClick={() => void updateItem(item, { quantity: item.quantity + 1 })}>+</button></div><strong>{formatRestaurantMoney(item.unit_price_cents * item.quantity)}</strong>{item.status === 'draft' && <button type="button" className="restaurant-order-remove" onClick={() => void removeDraftItem(item)} aria-label={`Supprimer ${item.item_name}`}>×</button>}<textarea disabled={item.status !== 'draft'} value={item.notes ?? ''} onChange={(event) => setItems((current) => current.map((row) => row.id === item.id ? { ...row, notes: event.target.value } : row))} onBlur={(event) => void updateItem(item, { notes: event.target.value.trim() || null })} placeholder="Cuisson, sans oignon, allergie…"/></article>)}</div>
          <div className="restaurant-order-total"><span>Total provisoire</span><strong>{formatRestaurantMoney(selectedOrder.total_cents)}</strong></div>
          <div className="restaurant-order-actions"><button type="button" className="primary-button" disabled={saving || !selectedItems.some((item) => item.status === 'draft') || selectedOrder.status === 'bill_requested'} onClick={() => void sendOrder()}><Icon name="utensils" size={17}/>{hasKitchen ? 'Envoyer en cuisine' : 'Valider la commande'}</button><button type="button" className="secondary-button" disabled={saving || selectedItems.length === 0 || selectedOrder.status === 'bill_requested'} onClick={() => void requestBill()}><Icon name="file" size={17}/>Préparer la note</button>{selectedOrder.status === 'bill_requested' && canClose && <button type="button" className="secondary-button" disabled={saving} onClick={() => void closeOrder()}><Icon name="check" size={17}/>Clôturer après règlement externe</button>}</div>
          <p className="restaurant-order-payment-note"><Icon name="creditCard" size={16}/>Le paiement est encaissé sur la caisse ou le terminal du restaurant. NCR Suite ne stocke aucun moyen de paiement.</p>
        </>}
      </aside>
    </section>
  </div>;
