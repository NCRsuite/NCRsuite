...
export function SecurityAgentsPage() {
  ...
  return <div className="page security-page">
    ...
    <section className="panel security-list-panel"><div className="security-toolbar"><div><p className="eyebrow">EFFECTIF</p><h2>{rows.length} agent{rows.length > 1 ? 's' : ''}</h2></div><input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Nom, matricule, e-mail…"/></div>
      {loading ? <div className="security-empty">Chargement…</div> : filtered.length === 0 ? <div className="security-empty"><Icon name="users" size={30}/><strong>Aucun agent</strong><span>Ajoute un agent pour commencer le planning.</span></div> : <div className="security-card-list">{filtered.map((row) => <article className="security-record-card security-agent-clickable-card" key={row.id}><span className="security-record-icon"><Icon name="shield" size={20}/></span><Link className="security-record-main security-agent-card-link" to={`/agents/${row.id}`}><strong>{securityPersonName(row.first_name, row.last_name)}</strong><span>{[row.employee_number && `Matricule ${row.employee_number}`, contractLabels[row.contract_type], `${Number(row.weekly_hours)} h/sem.`].filter(Boolean).join(' · ')}</span><small>{[row.email, row.phone].filter(Boolean).join(' · ') || 'Coordonnées à compléter'}</small></Link><span className={`security-status-pill ${row.linked_user_id ? 'completed' : 'active'}`}>{row.linked_user_id ? 'Connecté' : 'Interne'}</span><Link className="secondary-button compact-button" to={`/agents/${row.id}`}><Icon name="calendar" size={15}/>Planning</Link><button className="danger-text-button" type="button" onClick={() => void archive(row)}>Archiver</button></article>)}</div>}
    </section>
  </div>;
}
