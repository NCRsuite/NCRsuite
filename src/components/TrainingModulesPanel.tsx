...
export function TrainingModulesPanel() {
  ...
  async function requestChange(item: TrainingModuleCatalogItem, action: 'add' | 'remove') {
    ...
    const { data, error: requestError } = stripeManaged
      ? await supabase.rpc('request_stripe_addon_change', {
          ...
          p_item_key: item.module_key,
          p_action: action,
          p_accept_terms: true
        })
      : await supabase.rpc('request_training_module_change', {
          p_organization_id: organization.id,
  ...
  async function resumeStripeRequest(request: TrainingModuleRequest) {
    if (!organization || !supabase || !canManage) return;
              ...
              <article key={item.module_key} className={`security-addon-card${item.active ? ' active' : ''}${item.included_by_plan ? ' included' : ''}${lockedForPlan ? ' locked' : ''}${requestedFeature && item.feature_keys.includes(requestedFeature) ? ' requested' : ''}`}>
                ...
                {request ? (
                  <div className="security-addon-request">
                    ...
                    {request.provider === 'stripe' && <button type="button" className="primary-button compact-button" onClick={() => void resumeStripeRequest(request)} disabled={!canManage || busy === item.module_key}>Reprendre le paiement</button>}
                    {request.provider !== 'stripe' && <button type="button" className="secondary-button compact-button" onClick={() => void cancelRequest(request)} disabled={!canManage || busy === item.module_key}>Annuler</button>}
                  </div>
