...
export function MetierSharedReception({ onBack }: { onBack: () => void }) {
  ...
  async function searchClients(term = clientSearch) {
    if (!organization || !supabase) return;
    setBusy('clients');
    const { data, error: requestError } = await supabase.rpc('metier_reception_search_clients', {
      p_organization_id: organization.id,
  ...
  }, [companyId, companySites, companyServices]);

  useEffect(() => {
    if (!compatibleStaff.some((member) => member.id === staffId)) setStaffId(compatibleStaff[0]?.id ?? '');
  }, [compatibleStaff, staffId]);

  function resetForm() {
    const initialCompany = context?.companies[0]?.id ?? '';
  ...
  async function saveAppointment(event: FormEvent) {
    ...
    if (!organization || !supabase) return;
    if (!companyId || !siteId || !serviceId || !staffId) {
      setError('Choisissez l’entreprise, l’adresse, la prestation et le collaborateur.');
      return;
    }
    if (!newClient && !clientId) {
      setError('Choisissez un client ou créez-en un nouveau.');
      return;
    }
    if (newClient && firstName.trim().length < 2) {
      setError('Indiquez le prénom ou le nom du nouveau client.');
      return;
    }
    ...
    setMessage(`Rendez-vous enregistré pour ${selectedCompany?.name ?? 'l’entreprise'}.`);
    setShowForm(false);
    resetForm();
  ...
  return <div className="metier-simple-page metier-reception-page">
    <header className="metier-simple-hero reception-hero">
      <div><button className="metier-back-link" type="button" onClick={onBack}><Icon name="chevronLeft" size={16} /> Retour</button><p className="eyebrow">ACCUEIL PARTAGÉ</p><h1>Secrétariat</h1><p>Prenez les rendez-vous de toutes les entreprises autorisées depuis un seul écran, même si leur équipe n’est pas présente aujourd’hui.</p></div>
    ...
    {!loading && context && <>
      <section className="reception-company-strip">
        {context.companies.map((company) => {
          ...
          return <button type="button" key={company.id} className={company.id === companyId ? 'active' : ''} onClick={() => setCompanyId(company.id)}><span style={{ background: company.logo_url ? '#fff' : company.primary_color }}>{company.logo_url ? <img src={company.logo_url} alt="" /> : company.name.slice(0, 1).toUpperCase()}</span><span><strong>{company.name}</strong><small>{ready ? 'Disponible à la réservation' : 'Configuration à terminer'}</small></span></button>;
        })}
        {context.companies.length === 0 && <div className="metier-simple-empty"><Icon name="calendar" size={25} /><h3>Aucune entreprise activée</h3><p>Activez “Prise de rendez-vous” dans la configuration d’au moins une entreprise.</p></div>}
      </section>

      {showForm && <section className="metier-simple-card reception-form-card">
        <div className="metier-simple-heading"><div><p className="eyebrow">NOUVEAU RENDEZ-VOUS</p><h2>{selectedCompany?.name || 'Choisir une entreprise'}</h2><p>Le rendez-vous sera enregistré directement dans la bonne entreprise.</p></div><button className="icon-button" type="button" onClick={() => setShowForm(false)} aria-label="Fermer"><Icon name="close" size={20} /></button></div>
        <form className="reception-booking-form" onSubmit={saveAppointment}>
          <label>Entreprise<select value={companyId} onChange={(event) => setCompanyId(event.target.value)} required><option value="">Choisir…</option>{context.companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label>
          <label>Adresse / établissement<select value={siteId} onChange={(event) => setSiteId(event.target.value)} required><option value="">Choisir…</option>{companySites.map((site) => <option key={site.id} value={site.id}>{site.name}{site.city ? ` · ${site.city}` : ''}</option>)}</select></label>
          ...
          <div className="reception-client-block full">
            ...
            {!newClient ? <><div className="client-search-row"><input value={clientSearch} onChange={(event) => setClientSearch(event.target.value)} placeholder="Nom, téléphone ou e-mail" /><button type="button" className="secondary-button compact-button" onClick={() => void searchClients()} disabled={busy === 'clients'}><Icon name="search" size={15} /> Rechercher</button></div><div className="client-result-list">{clients.map((client) => <button type="button" key={client.id} className={client.id === clientId ? 'active' : ''} onClick={() => setClientId(client.id)}><span><strong>{[client.first_name, client.last_name].filter(Boolean).join(' ')}</strong><small>{client.phone || client.email || 'Client'}</small></span>{client.id === clientId && <Icon name="check" size={17} />}</button>)}</div></> : <div className="new-client-grid"><label>Prénom / nom<input value={firstName} onChange={(event) => setFirstName(event.target.value)} required /></label><label>Nom complémentaire<input value={lastName} onChange={(event) => setLastName(event.target.value)} /></label><label>Téléphone<input value={phone} onChange={(event) => setPhone(event.target.value)} /></label><label>E-mail<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label></div>}
          </div>
      ...
      <section className="metier-simple-section reception-agenda-section">
        <div className="metier-simple-heading"><div><p className="eyebrow">AGENDA COMMUN</p><h2>Prochains rendez-vous</h2><p>Toutes les entreprises autorisées sont réunies ici.</p></div><button className="secondary-button" type="button" onClick={() => void loadAppointments()}><Icon name="refresh" size={16} /> Actualiser</button></div>
        <div className="reception-appointment-list">
          {upcoming.map((appointment) => <article key={appointment.id}><div className="appointment-time"><strong>{new Intl.DateTimeFormat('fr-FR', { day: '2-digit', month: 'short' }).format(new Date(appointment.starts_at))}</strong><span>{new Intl.DateTimeFormat('fr-FR', { hour: '2-digit', minute: '2-digit' }).format(new Date(appointment.starts_at))}</span></div><div className="appointment-company"><strong>{appointment.company_name}</strong><small>{appointment.site_name || 'Établissement'} · {appointment.service_name}</small></div><div className="appointment-client"><strong>{appointment.client_name}</strong><small>{appointment.client_phone || appointment.staff_name}</small></div><span className={`appointment-status ${appointment.status}`}>{statusLabels[appointment.status] || appointment.status}</span><div className="appointment-actions">{appointment.status === 'confirmed' && <><button type="button" onClick={() => void updateStatus(appointment, 'completed')} disabled={busy === `status-${appointment.id}`}>Terminé</button><button type="button" className="danger" onClick={() => void updateStatus(appointment, 'cancelled')} disabled={busy === `status-${appointment.id}`}>Annuler</button></>}</div></article>)}
