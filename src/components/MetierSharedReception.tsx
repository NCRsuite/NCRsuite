...
export function MetierSharedReception({ onBack }: { onBack: () => void }) {
  ...
  async function loadContext() {
    ...
    setLoading(true);
    setError('');
    const { data, error: requestError } = await supabase.rpc('metier_reception_context', {
    ...
    const next = (data ?? {
      companies: [], sites: [], services: [], staff: [], staff_services: []
    }) as ReceptionContext;
  ...
  const companySites = useMemo(
    () => (context?.sites ?? []).filter((site) => site.company_id === companyId),
    [context?.sites, companyId]
  );
  const companyServices = useMemo(
    () => (context?.services ?? []).filter((service) => service.company_id === companyId),
    [context?.services, companyId]
  );
  const compatibleStaff = useMemo(() => {
    const allowed = new Set(
      (context?.staff_services ?? [])
        .filter((item) => !serviceId || item.service_id === serviceId)
        .map((item) => item.staff_id)
    );
    return (context?.staff ?? []).filter((member) =>
      member.company_id === companyId
      && (!siteId || !member.site_id || member.site_id === siteId)
      && (!serviceId || allowed.has(member.id))
    );
  ...
  const selectedCompany = useMemo(
    ...
    [context?.companies, companyId]
  );
  const upcoming = useMemo(
    () => appointments.filter((appointment) => appointment.status !== 'cancelled').slice(0, 50),
    [appointments]
  );
  ...
  function resetForm() {
    setCompanyId(context?.companies[0]?.id ?? '');
    setSiteId('');
    setServiceId('');
    setStaffId('');
    setClientId('');
  ...
  return (
    <div className="metier-simple-page metier-reception-page">
      <header className="metier-simple-hero reception-hero">
        <div>
          <button className="metier-back-link" type="button" onClick={onBack}>
            <Icon name="chevronRight" size={16} /> Retour
          </button>
          <p className="eyebrow">ACCUEIL PARTAGÉ</p>
          <h1>Secrétariat</h1>
          <p>Prenez les rendez-vous de toutes les entreprises autorisées depuis un seul écran, même si leur équipe n’est pas présente aujourd’hui.</p>
        </div>
        <button className="primary-button" type="button" onClick={() => setShowForm(true)} disabled={!context?.companies.length}>
          <Icon name="plus" size={17} /> Nouveau rendez-vous
        </button>
      </header>
        ...
        <>
          <section className="reception-company-strip">
            {context.companies.map((company) => {
              ...
              return (
                <button type="button" key={company.id} className={company.id === companyId ? 'active' : ''} onClick={() => setCompanyId(company.id)}>
                  <span style={{ background: company.logo_url ? '#fff' : company.primary_color }}>
                    {company.logo_url ? <img src={company.logo_url} alt="" /> : company.name.slice(0, 1).toUpperCase()}
                  </span>
                  <span>
                    <strong>{company.name}</strong>
                    <small>{ready ? 'Disponible à la réservation' : 'Configuration à terminer'}</small>
                  </span>
                </button>
              );
            })}
            {context.companies.length === 0 && (
              <div className="metier-simple-empty">
                ...
                <h3>Aucune entreprise activée</h3>
                <p>Activez « Prise de rendez-vous » dans la configuration d’au moins une entreprise.</p>
              </div>
            )}
          </section>

          {showForm && (
            <section className="metier-simple-card reception-form-card">
              <div className="metier-simple-heading">
                <div>
                  <p className="eyebrow">NOUVEAU RENDEZ-VOUS</p>
                  <h2>{selectedCompany?.name || 'Choisir une entreprise'}</h2>
                  <p>Le rendez-vous sera enregistré directement dans la bonne entreprise.</p>
                </div>
                <button className="icon-button" type="button" onClick={() => setShowForm(false)} aria-label="Fermer">
              ...
              <form className="reception-booking-form" onSubmit={saveAppointment}>
                ...
                <label>
                  ...
                  <select value={siteId} onChange={(event) => setSiteId(event.target.value)} required>
                    <option value="">Choisir…</option>
                    {companySites.map((site) => <option key={site.id} value={site.id}>{site.name}{site.city ? ` · ${site.city}` : ''}</option>)}
                  </select>
                </label>
                <label>
                  Prestation
                  <select value={serviceId} onChange={(event) => setServiceId(event.target.value)} required>
                    <option value="">Choisir…</option>
                    {companyServices.map((service) => <option key={service.id} value={service.id}>{service.name} · {service.duration_minutes} min · {money(service.price_cents)}</option>)}
                  </select>
                </label>
                <label>
                  Collaborateur
                ...
                <div className="reception-client-block full">
                  <div className="reception-client-choice">
                    <button type="button" className={!newClient ? 'active' : ''} onClick={() => setNewClient(false)}>Client existant</button>
                    <button type="button" className={newClient ? 'active' : ''} onClick={() => { setNewClient(true); setClientId(''); }}>Nouveau client</button>
                  </div>

                  {!newClient ? (
                    <>
                      <div className="client-search-row">
                        <input value={clientSearch} onChange={(event) => setClientSearch(event.target.value)} placeholder="Nom, téléphone ou e-mail" />
                        <button type="button" className="secondary-button compact-button" onClick={() => void searchClients()} disabled={busy === 'clients'}>
                          <Icon name="search" size={15} /> Rechercher
                        </button>
                      </div>
                      <div className="client-result-list">
                        {clients.map((client) => (
                          <button type="button" key={client.id} className={client.id === clientId ? 'active' : ''} onClick={() => setClientId(client.id)}>
                            <span>
                              <strong>{[client.first_name, client.last_name].filter(Boolean).join(' ')}</strong>
                              <small>{client.phone || client.email || 'Client'}</small>
                            </span>
                            {client.id === clientId && <Icon name="check" size={17} />}
                          </button>
                        ))}
                      </div>
                    </>
                    ...
                    <div className="new-client-grid">
                      <label>Prénom / nom<input value={firstName} onChange={(event) => setFirstName(event.target.value)} required /></label>
                      <label>Nom complémentaire<input value={lastName} onChange={(event) => setLastName(event.target.value)} /></label>
                ...
                <div className="metier-form-actions full">
                  ...
                  <button type="submit" className="primary-button" disabled={busy === 'appointment'}>
                    {busy === 'appointment' ? 'Enregistrement…' : 'Confirmer le rendez-vous'}
                  </button>
                </div>
              </form>
            </section>
          ...
          <section className="metier-simple-section reception-agenda-section">
            <div className="metier-simple-heading">
              <div>
                <p className="eyebrow">AGENDA COMMUN</p>
                <h2>Prochains rendez-vous</h2>
                <p>Toutes les entreprises autorisées sont réunies ici.</p>
              </div>
              ...
              {upcoming.map((appointment) => (
                <article key={appointment.id}>
                  <div className="appointment-time">
                    <strong>{new Intl.DateTimeFormat('fr-FR', { day: '2-digit', month: 'short' }).format(new Date(appointment.starts_at))}</strong>
                    <span>{new Intl.DateTimeFormat('fr-FR', { hour: '2-digit', minute: '2-digit' }).format(new Date(appointment.starts_at))}</span>
                  </div>
                  <div className="appointment-company">
                    <strong>{appointment.company_name}</strong>
                    <small>{appointment.site_name || 'Établissement'} · {appointment.service_name}</small>
                  </div>
                  <div className="appointment-client">
                    <strong>{appointment.client_name}</strong>
                    <small>{appointment.client_phone || appointment.staff_name}</small>
                  </div>
                  <span className={`appointment-status ${appointment.status}`}>{statusLabels[appointment.status] || appointment.status}</span>
                  <div className="appointment-actions">
                    {appointment.status === 'confirmed' && (
                      <>
                        <button type="button" onClick={() => void updateStatus(appointment, 'completed')} disabled={busy === `status-${appointment.id}`}>Terminé</button>
                        <button type="button" className="danger" onClick={() => void updateStatus(appointment, 'cancelled')} disabled={busy === `status-${appointment.id}`}>Annuler</button>
                      </>
                    )}
                  </div>
                </article>
